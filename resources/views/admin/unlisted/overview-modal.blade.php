...
.ov-save-btn:hover { background:#054d3c; }
...
.ov-field textarea:focus {
    border-color:#076550;
    box-shadow:0 0 0 3px rgba(7, 101, 80, 0.12);
}
...
.ov-radios input[type=radio] { accent-color:#076550; width:15px; height:15px; }
...
/* TinyMCE dialogs must float above the modal overlay (z-index: 2100) */
.tox-tinymce-aux,
...
<div class="ov-modal">
    ...
    <form id="overviewForm" data-fincode="{{ $stock->UL_STOCKS_FINCODE }}">
        ...
        <div class="ov-body">
            ...
            {{-- Row: Slug | ISIN | Short Name | Category --}}
...
(function () {
    ...
    tinymce.init({
        selector: 'textarea#OV_ABOUT_EDITOR',
    ...
    // Live-format the Slug field: lowercase, spaces -> hyphen, strip special chars
    $('#OV_SLUG_INPUT').on('input', function () {
        ...
        this.selectionStart = this.selectionEnd = pos + (after - before);
    }).on('blur', function () {
