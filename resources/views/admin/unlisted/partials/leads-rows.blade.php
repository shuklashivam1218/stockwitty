...
<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-top:1px solid #f1f3f5;background:#fafafa;">
    ...
    <div style="display:flex;gap:4px;">
        ...
        <button onclick="loadLeads({{ $p }})"
            style="padding:5px 10px;border:1px solid {{ $p === $leads->currentPage() ? '#076550' : '#e5e7eb' }};border-radius:6px;background:{{ $p === $leads->currentPage() ? '#076550' : '#fff' }};font-size:12px;cursor:pointer;color:{{ $p === $leads->currentPage() ? '#fff' : '#374151' }};font-weight:{{ $p === $leads->currentPage() ? '600' : '400' }};">
            {{ $p }}
        </button>
        @endfor
