    ...
    <tr style="border-bottom:1px solid #f1f3f5;transition:background 0.12s;"
        ...
        <td style="padding:12px 12px;vertical-align:middle;text-align:center;white-space:nowrap;">
            ...
            <button class="open-edit-ord"
                ...
                onmouseover="this.style.borderColor='#076550';this.style.color='#076550';this.style.background='#e8f5f1';"
...
<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-top:1px solid #f1f3f5;background:#fafafa;">
    ...
    <div style="display:flex;gap:4px;">
        <button onclick="loadOrders({{ $orders->currentPage() - 1 }})" {{ $orders->onFirstPage() ? 'disabled' : '' }}
            ...
            onmouseover="if(!this.disabled)this.style.borderColor='#076550';" onmouseout="this.style.borderColor='#e5e7eb';">
            ‹ Prev
        </button>
        ...
        <button onclick="loadOrders({{ $p }})"
            style="padding:5px 10px;border:1px solid {{ $p === $orders->currentPage() ? '#076550' : '#e5e7eb' }};border-radius:6px;background:{{ $p === $orders->currentPage() ? '#076550' : '#fff' }};font-size:12px;cursor:pointer;color:{{ $p === $orders->currentPage() ? '#fff' : '#374151' }};font-weight:{{ $p === $orders->currentPage() ? '600' : '400' }};">
            {{ $p }}
        </button>
        @endfor
