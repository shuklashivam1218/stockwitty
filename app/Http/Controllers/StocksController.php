...
class StocksController extends Controller
{
    ...
    public function company(string $slug)
    {
        ...
        // About content is authored via the Overview modal (UL_STOCKS_ABOUT), not the separate About modal.
        $aboutHtml  = $fixImageSrc($stock->UL_STOCKS_ABOUT);
        ...
        $priceHistory = DB::table('unlisted_price_data')
            ->where('UL_PD_FINCODE', $fincode)
            ->where('UL_PD_INVALID_FLAG', 0)
            ->where('UL_PD_DATE', '>=', now()->subYears(5))
            ->orderBy('UL_PD_DATE')
            ->get(['UL_PD_DATE', 'UL_PD_BID_PRICE'])
            ->map(fn ($r) => [
                'date'  => Carbon::parse($r->UL_PD_DATE)->format('Y-m-d'),
                'price' => (float) $r->UL_PD_BID_PRICE,
            ])
            ->values();

        $similarCompanies = collect();
        if ($stock->UL_STOCKS_IND_CODE) {
            $similarCompanies = collect(DB::select("
                SELECT
                    s.UL_STOCKS_FINCODE,
                    s.UL_STOCKS_COMPNAME,
                    s.UL_STOCKS_SLUG,
                ...
                WHERE s.UL_STOCKS_STATUS = '1'
                  AND s.UL_STOCKS_IND_CODE = ?
                  AND s.UL_STOCKS_FINCODE != ?
                LIMIT 6
            ", [$stock->UL_STOCKS_IND_CODE, $fincode]));
        }

        $faqsByTarget = UnlistedFaq::where('UL_FAQ_FINCODE', $fincode)
            ->where('UL_FAQ_ACTIVE', '1')
            ->orderBy('UL_FAQ_SORT_ORDER')
            ->get()
            ->groupBy('UL_FAQ_TARGET');

        $overviewFaqs = $faqsByTarget->get('overview', collect());
        $aboutFaqs    = $faqsByTarget->get('about', collect());
        $thesisFaqs   = $faqsByTarget->get('thesis', collect());
        ...
        $priceAsOf = $priceData?->UL_PD_DATE;

        // YoY growth vs the previous available fiscal year (both already fetched in $financials).
        $prevYearFin      = $financials->skip(1)->first();
        $netSalesGrowthPct = null;
        $patGrowthPct      = null;
        if ($latestFin && $prevYearFin) {
            $prevSales = (float) $prevYearFin->UL_FIN_NET_SALES;
            $prevPat   = (float) $prevYearFin->UL_FIN_PAT;
            if ($prevSales != 0) {
                $netSalesGrowthPct = round((((float) $latestFin->UL_FIN_NET_SALES - $prevSales) / abs($prevSales)) * 100, 1);
            }
            if ($prevPat != 0) {
                $patGrowthPct = round((((float) $latestFin->UL_FIN_PAT - $prevPat) / abs($prevPat)) * 100, 1);
            }
        }

        // Revenue & PAT trend (Cr) across fetched yearly periods, oldest first — for a small trend chart.
        ...
        return view('public.company', compact(
            ...
            'aboutHtml', 'currentPrice', 'marketCap', 'peRatio', 'eps', 'bookValue',
            'pbRatio', 'roe', 'debtToEquity', 'high52w', 'low52w', 'weekChange', 'weekChangePct',
            'priceHistory', 'similarCompanies', 'priceAsOf', 'netSalesGrowthPct', 'patGrowthPct',
            'revenueTrend', 'overviewFaqs', 'aboutFaqs', 'thesisFaqs'
        ));
    }
}
